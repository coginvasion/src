# Embedded file name: lib.coginvasion.toon.DistributedHQNPCToonAI
...
class DistributedHQNPCToonAI(DistributedNPCToonAI.DistributedNPCToonAI):
    ...
    def hasValidReasonToEnter(self, avId):
        ...
        if av:
            chat = None
            if len(av.getQuests()[0]) >= 4:
                array = CIGlobals.NPCEnter_Pointless_Dialogue
                array.remove(array[28])
                chat = random.choice(array)
            elif len(av.questManager.getPickableQuestList(self)) == 0 and not av.questManager.hasAnObjectiveToVisit(self.npcId, self.zoneId):
