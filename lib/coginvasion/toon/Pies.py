# Embedded file name: lib.coginvasion.toon.Pies
"""
...
class Pies:
    ...
    def __init__(self):
        ...
        self.current_ammo = {1: 7,
         ...
         2: 15,
         3: 0}
        self.weapon_id_2_weapon = {0: 'cake',
         1: 'tart',
         2: 'slice',
         3: 'tnt'}
        self.avatar = None
    ...
    def tntRelease(self):
        if self.tnt is None:
            ...
            tntNp.setHpr(0, 90, 0)
            self.tntTrajectory = ProjectileInterval(self.tnt, startPos=self.avatar.getPart('torso').find('**/def_joint_right_hold').getPos(render), endPos=tntNp.getPos(render), gravityMult=0.9, duration=3)
    ...
    def handleTntHitGround(self):
        if not self.tnt:
            ...
            self.tntSparks.start(parent=self.tnt.find('**/joint_attachEmitter'), renderParent=self.tnt.find('**/joint_attachEmitter'))
    ...
    def pieRelease(self):
        if self.pie is None:
            ...
            self.trajectory = ProjectileInterval(self.pie, startPos=self.avatar.getPart('torso').find('**/def_joint_right_hold').getPos(render), endPos=self.pieNp.getPos(render), gravityMult=0.9, duration=3)
