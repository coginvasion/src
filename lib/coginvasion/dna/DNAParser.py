# Embedded file name: lib.coginvasion.dna.DNAParser
...
tokens = ['FLOAT',
 ...
 'UNQUOTED_STRING',
 'QUOTED_STRING']
reserved = {'store_suit_point': 'STORE_SUIT_POINT',
 'group': 'GROUP',
 'visgroup': 'VISGROUP',
 'vis': 'VIS',
...
t_ignore_COMMENT.__doc__ = '[/]{2,2}.*'

def t_ignore_ML_COMMENT(t):
    pass


t_ignore_ML_COMMENT.__doc__ = '\\/\\*([^*]|[\\r\\n])*\\*/'
...
class DNASuitPoint():
    ...
    def __str__(self):
        ...
        return 'DNASuitPoint index: {0}, pointType: {1}, pos: {2}'.format(self.getIndex(), pointTypeStr, self.getPos())
...
class DNAWindows(DNAGroup):
    ...
    def setupWindows(parentNode, dnaStorage, code, windowCount, color, hpr, scale):
        ...
        if node_r is None or node_l is None:
            raise DNAError('DNAWindows code {0} not found inDNAStorage'.format(code))
        ...
        if windowCount == 1:
            ...
            raise NotImplementedError('Invalid window count {0}'.format(windowCount))
        return
...
class DNACornice(DNAGroup):
    ...
    def traverse(self, nodePath, dnaStorage):
        ...
        if node is None:
            raise DNAError('DNACornice code {0} not found in DNAStorage'.format(self.getCode()))
...
p_dna.__doc__ = 'dna : dna object\n           | object'

def p_object(p):
    p[0] = p[1]


p_object.__doc__ = 'object : suitpoint\n              | group\n              | model\n              | font\n              | store_texture'

def p_number(p):
    p[0] = p[1]


p_number.__doc__ = 'number : FLOAT\n              | INTEGER'
...
def p_suitpoint(p):
    ...
    point = DNASuitPoint(p[3], p[5], p[7], landmarkBuildingIndex=landmarkBuildingIndex)
    p.parser.dnaStore.storeSuitPoint(point)


p_suitpoint.__doc__ = 'suitpoint : STORE_SUIT_POINT "[" number "," suitpointtype "," lpoint3f "]"\n                 | STORE_SUIT_POINT "[" number "," suitpointtype "," lpoint3f "," number "]"'

def p_suitpointtype(p):
    p[0] = DNASuitPoint.pointTypeMap[p[1]]


p_suitpointtype.__doc__ = 'suitpointtype : STREET_POINT\n                     | FRONT_DOOR_POINT\n                     | SIDE_DOOR_POINT\n                     | COGHQ_IN_POINT\n                     | COGHQ_OUT_POINT'

def p_string(p):
    p[0] = p[1]


p_string.__doc__ = 'string : QUOTED_STRING\n              | UNQUOTED_STRING'
...
p_string_opt_list.__doc__ = 'string_opt_list : string_opt_list string\n                       | empty'
...
p_group.__doc__ = 'group : dnagroup\n             | visgroup\n             | dnanode\n             | windows\n             | cornice\n             | door'

def p_dnanode(p):
    p[0] = p[1]


p_dnanode.__doc__ = 'dnanode : prop\n               | sign\n               | signbaseline\n               | signtext\n               | flatbuilding\n               | wall\n               | landmarkbuilding\n               | street\n               | signgraphic\n               | dnanode_grp'
...
p_door.__doc__ = 'door : doordef "[" subdoor_list "]"\n            | flatdoordef "[" subdoor_list "]"'
...
p_subgroup_list.__doc__ = 'subgroup_list : subgroup_list group\n                     | empty'
...
p_hpr.__doc__ = 'hpr : HPR "[" lpoint3f "]"\n           | NHPR "[" lpoint3f "]"'
...
p_dnanode_sub.__doc__ = 'dnanode_sub : group\n                   | pos\n                   | hpr\n                   | scale'

def p_dnaprop_sub(p):
    p[0] = p[1]


p_dnaprop_sub.__doc__ = 'dnaprop_sub : code\n                   | color'
...
p_baseline_sub.__doc__ = 'baseline_sub : code\n                    | color\n                    | width\n                    | height\n                    | indent\n                    | kern\n                    | stomp\n                    | stumble\n                    | wiggle\n                    | flags'
...
p_signgraphic_sub.__doc__ = 'signgraphic_sub : width\n                       | height\n                       | code\n                       | color'
...
p_wall_sub.__doc__ = 'wall_sub : height\n                | code\n                | color'

def p_windows_sub(p):
    p[0] = p[1]


p_windows_sub.__doc__ = 'windows_sub : code\n                   | color\n                   | windowcount'

def p_cornice_sub(p):
    p[0] = p[1]


p_cornice_sub.__doc__ = 'cornice_sub : code\n                   | color'

def p_landmarkbuilding_sub(p):
    p[0] = p[1]


p_landmarkbuilding_sub.__doc__ = 'landmarkbuilding_sub : code\n                            | title\n                            | article\n                            | building_type\n                            | wall_color'

def p_animbuilding_sub(p):
    p[0] = p[1]


p_animbuilding_sub.__doc__ = 'animbuilding_sub : anim'

def p_door_sub(p):
    p[0] = p[1]


p_door_sub.__doc__ = 'door_sub : code\n                | color'

def p_street_sub(p):
    p[0] = p[1]


p_street_sub.__doc__ = 'street_sub : code\n                  | texture\n                  | color'
...
def p_wall_color(p):
    p.parser.parentGroup.setWallColor((p[3],
     p[4],
     p[5],
     p[6]))
...
def p_color(p):
    p.parser.parentGroup.setColor((p[3],
     p[4],
     p[5],
     p[6]))
...
p_subprop_list.__doc__ = 'subprop_list : subprop_list dnanode_sub\n                    | subprop_list dnaprop_sub\n                    | empty'
...
p_subanimprop_list.__doc__ = 'subanimprop_list : subanimprop_list dnanode_sub\n                        | subanimprop_list dnaprop_sub\n                        | subanimprop_list dnaanimprop_sub\n                        | empty'
...
p_subbaseline_list.__doc__ = 'subbaseline_list : subbaseline_list dnanode_sub\n                        | subbaseline_list baseline_sub\n                        | empty'
...
p_subtext_list.__doc__ = 'subtext_list : subtext_list dnanode_sub\n                    | subtext_list text_sub\n                    | empty'
...
p_subdnanode_list.__doc__ = 'subdnanode_list : subdnanode_list dnanode_sub\n                       | empty'
...
p_subsigngraphic_list.__doc__ = 'subsigngraphic_list : subsigngraphic_list dnanode_sub\n                           | subsigngraphic_list signgraphic_sub\n                           | empty'
...
p_subflatbuilding_list.__doc__ = 'subflatbuilding_list : subflatbuilding_list dnanode_sub\n                            | subflatbuilding_list flatbuilding_sub\n                            | empty'
...
p_subwall_list.__doc__ = 'subwall_list : subwall_list dnanode_sub\n                    | subwall_list wall_sub\n                    | empty'
...
p_subwindows_list.__doc__ = 'subwindows_list : subwindows_list dnanode_sub\n                       | subwindows_list windows_sub\n                       | empty'
...
p_subcornice_list.__doc__ = 'subcornice_list : subcornice_list dnanode_sub\n                       | subcornice_list cornice_sub\n                       | empty'
...
p_sublandmarkbuilding_list.__doc__ = 'sublandmarkbuilding_list : sublandmarkbuilding_list dnanode_sub\n                                | sublandmarkbuilding_list landmarkbuilding_sub\n                                | empty'
...
p_subanimbuilding_list.__doc__ = 'subanimbuilding_list : subanimbuilding_list dnanode_sub\n                            | subanimbuilding_list landmarkbuilding_sub\n                            | subanimbuilding_list animbuilding_sub\n                            | empty'
...
p_subdoor_list.__doc__ = 'subdoor_list : subdoor_list dnanode_sub\n                    | subdoor_list door_sub\n                    | empty'
...
p_substreet_list.__doc__ = 'substreet_list : substreet_list dnanode_sub\n                      | substreet_list street_sub\n                      | empty'
...
p_modeldef.__doc__ = 'modeldef : MODEL string\n                | HOODMODEL string\n                | PLACEMODEL string'
...
p_modelnode_list.__doc__ = 'modelnode_list : modelnode_list node\n                      | empty'
...
p_node.__doc__ = 'node : STORE_NODE "[" string string "]"\n            | STORE_NODE "[" string string string "]"'
...
p_store_texture.__doc__ = 'store_texture : STORE_TEXTURE "[" string string "]"\n                     | STORE_TEXTURE "[" string string string "]"'
...
p_font.__doc__ = 'font : STORE_FONT "[" string string string "]"'
