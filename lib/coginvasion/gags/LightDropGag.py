# Embedded file name: lib.coginvasion.gags.LightDropGag
"""
...
class LightDropGag(DropGag):
    ...
    def startDrop(self):
        if self.gag and self.dropLoc:
            ...
            shadowTrack = Sequence(LerpScaleInterval(dropShadow, self.fallDuration + 0.1, dropShadow.getScale(), startScale=Point3(0.01, 0.01, 0.01)), Wait(0.3), Func(dropShadow.removeNode))
    ...
    def onActivate(self, ignore, suit):
        ...
        dummyNode.setX(2)
        dummyNode.setY(-2)
        flightIval = FlightProjectileInterval(self.gag, startPos=self.gag.getPos(render), endPos=dummyNode.getPos(render), duration=0.8, gravityMult=0.35)
