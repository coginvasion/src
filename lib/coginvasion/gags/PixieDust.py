# Embedded file name: lib.coginvasion.gags.PixieDust
"""
...
from direct.interval.IntervalGlobal import Parallel, Sequence, Func, ActorInterval, Wait, ParticleInterval
...
class PixieDust(ToonUpGag):
    ...
    def doPixieDust(self, target):
        ...
        delay = 2.5
        track = Sequence()
        mtrack = Parallel(self.__getPartTrack(sprayEffect, 1.5, 0.5, [sprayEffect, self.avatar, 0]), self.__getPartTrack(dropEffect, 1.9, 2.0, [dropEffect, target, 0]), self.__getPartTrack(explodeEffect, 2.7, 1.0, [explodeEffect, self.avatar, 0]), self.__getPartTrack(poofEffect, 3.4, 1.0, [poofEffect, target, 0]), self.__getPartTrack(wallEffect, 4.05, 1.2, [wallEffect, self.avatar, 0]), self.getSoundTrack(0, self.avatar, 4.1), Sequence(Func(face90), ActorInterval(self.avatar, 'sprinkle-dust')), Sequence(Wait(delay), Func(doHeal)))
        track.append(mtrack)
