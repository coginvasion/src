...
class DistributedDoor(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.rightFSM = ClassicFSM('DistributedDoor_right', [State('off', self.enterOff, self.exitOff),
        ...
        self.leftFSM = ClassicFSM('DistributedDoor_left', [State('off', self.enterOff, self.exitOff),
    ...
    def getLeftDoorOpenH(self):
        ...
        if self.getDoorType() == self.INT_STANDARD or self.getDoorType() == self.INT_HQ:
            ...
            num = -110
        if self.getDoorIndex() == 1 and not self.doorType == self.EXT_HQ or self.getDoorIndex() == 0 and self.doorType == self.EXT_HQ:
            return num - 180
        else:
            return num
    ...
    def enterRightDoorClosing(self, ts = 0):
        if self.rightTrack:
            ...
            self.rightTrack = None
        self.rightTrack = Sequence(LerpQuatInterval(self.rightDoor, duration=1.0, quat=(self.getRightDoorClosedH(), 0, 0), startHpr=(self.getRightDoorOpenH(), 0, 0), blendType='easeIn'), Func(base.playSfx, self.doorShutSound, 0, 1, None, 0.0, self.rightDoor))
    ...
    def enterRightDoorOpening(self, ts = 0):
        if self.rightTrack:
            self.rightTrack.finish()
            self.rightTrack = None
        self.rightTrack = Sequence(Wait(0.5), Parallel(LerpQuatInterval(self.rightDoor, duration=0.7, quat=(self.getRightDoorOpenH(), 0, 0), startHpr=(self.getRightDoorClosedH(), 0, 0), blendType='easeInOut'), SoundInterval(self.doorOpenSound, node=self.rightDoor)))
        self.rightTrack.start()
        return
    ...
    def enterLeftDoorClosing(self, ts = 0):
        if self.leftTrack:
            ...
            self.leftTrack = None
        self.leftTrack = Sequence(LerpQuatInterval(self.leftDoor, duration=1.0, quat=(self.getLeftDoorClosedH(), 0, 0), startHpr=(self.getLeftDoorOpenH(), 0, 0), blendType='easeIn'), Func(base.playSfx, self.doorShutSound, 0, 1, None, 0.0, self.leftDoor))
    ...
    def enterLeftDoorOpening(self, ts = 0):
        if self.leftTrack:
            self.leftTrack.finish()
            self.leftTrack = None
        self.leftTrack = Sequence(Wait(0.5), Parallel(LerpQuatInterval(self.leftDoor, duration=0.7, quat=(self.getLeftDoorOpenH(), 0, 0), startHpr=(self.getLeftDoorClosedH(), 0, 0), blendType='easeInOut'), SoundInterval(self.doorOpenSound, node=self.leftDoor)))
        self.leftTrack.start()
        return
    ...
    def generated(self):
        ...
        self.doorOpenSound = base.loadSfx('phase_3.5/audio/sfx/Door_Open_1.mp3')
        self.doorShutSound = base.loadSfx('phase_3.5/audio/sfx/Door_Close_1.mp3')
