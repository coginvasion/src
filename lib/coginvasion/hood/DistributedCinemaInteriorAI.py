# Embedded file name: lib.coginvasion.hood.DistributedCinemaInteriorAI
...
class DistributedCinemaInteriorAI(DistributedToonInteriorAI.DistributedToonInteriorAI):
    ...
    def __init__(self, air, block, doorToZone, cinemaIndex):
        ...
        self.fsm = ClassicFSM.ClassicFSM('DCinemaInteriorAI', [State.State('off', self.enterOff, self.exitOff), State.State('show', self.enterShow, self.exitShow), State.State('intermission', self.enterIntermission, self.exitIntermission)], 'off', 'off')
    ...
    def enterShow(self):
        base.taskMgr.doMethodLater(CinemaGlobals.CinemaLengths[self.cinemaIndex], self.__showOver, self.uniqueName('showOver'))
    ...
    def enterIntermission(self):
        base.taskMgr.doMethodLater(CinemaGlobals.IntermissionLength, self.__intermissionOver, self.uniqueName('intermissionOver'))
