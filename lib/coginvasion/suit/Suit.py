# Embedded file name: lib.coginvasion.suit.Suit
"""
...
class Suit(Avatar.Avatar):
    ...
    def setChat(self, chatString):
        ...
        if self.isSkele:
            ...
            self.suit_dial = base.audio3d.loadSfx(random.choice(['phase_3.5/audio/dial/COG_VO_question.mp3', 'phase_3.5/audio/dial/COG_VO_question_2.mp3']))
        elif '!' in chatString:
    ...
    def enterDrop(self, ts = 0):
        self.suitTrack = Parallel(ActorInterval(self, 'drop'))
    ...
    def enterDie(self, ts = 0):
        ...
        self.suitTrack = Parallel(Sequence(Wait(0.7), Func(self.doSingleGear), Wait(4.3), Func(self.suitExplode), Wait(1.0), Func(self.delSuit)), gearTrack, name=trackName)
    ...
    def enterFlyNeutral(self, ts = 0):
        ...
        self.prop = Actor('phase_4/models/props/propeller-mod.bam', {'chan': 'phase_4/models/props/propeller-chan.bam'})
    ...
    def enterFlyDown(self, ts = 0):
        ...
        if hasattr(self, 'uniqueName'):
            ...
            name = 'enterFlyDown'
        self.suitTrack = Parallel(Sequence(Func(self.pose, 'land', 0), Func(self.prop.loop, 'chan', fromFrame=0, toFrame=3), Wait(1.75), Func(self.prop.play, 'chan', fromFrame=3), Wait(0.15), ActorInterval(self, 'land', duration=dur)), name=name)
    ...
    def enterFlyAway(self, ts = 0):
        ...
        if hasattr(self, 'uniqueName'):
            ...
            name = 'enterFlyAway'
        self.suitTrack = Sequence(Func(self.prop.play, 'chan', fromFrame=3), Wait(1.75), Func(self.prop.play, 'chan', fromFrame=0, toFrame=3), name=name)
        self.suitTrack.setDoneEvent(self.suitTrack.getName())
        self.acceptOnce(self.suitTrack.getDoneEvent(), self.exitFlyAway)
        self.suitTrack.delayDelete = DelayDelete.DelayDelete(self, name)
        self.suitTrack.start(ts)
