# Embedded file name: lib.coginvasion.suit.VicePresident
"""
...
class VicePresident(Avatar):
    ...
    def enterRiseUp(self, ts = 0):
        ...
        self.track = Sequence(Func(base.playSfx, sfx), ActorInterval(self, 'up'), name=name)
        self.knockedDown = False
    ...
    def enterKnockDown(self, ts = 0):
        ...
        base.audio3d.attachSoundToObject(sfx, self)
        self.track = Sequence(Func(base.playSfx, sfx), ActorInterval(self, 'fall'), name=name)
    ...
    def enterEmerge(self, ts = 0):
        ...
        self.track = Sequence(Func(base.playSfx, emergeSfx), LerpScaleInterval(self, duration=1.2, scale=1.0, startScale=0.05, blendType='easeOut'), name=name)
        self.track.setDoneEvent(self.track.getName())
        self.acceptOnce(self.track.getDoneEvent(), self.fsm.request, ['neutral'])
        self.track.start(ts)
    ...
    def enterNeutral(self, ts = 0):
        if self.getCurrentAnim() != 'stand-angry':
            if self.knockedDown:
                ...
                self.loop('stand-angry')
        self.track = NPCLookInterval(self.vp_torso_node, Vec3(0, 0, 0), blendType='easeInOut', name='lookAtCenter', isBackwards=False)
        self.track.start(ts)
    ...
    def enterJump(self, ts = 0):
        ...
        self.track = Sequence(Func(self.play, 'jump'), Func(base.playSfx, jumpSfx), Wait(1.2), Func(base.playSfx, landSfx), Wait(1.8), name=name)
        self.track.setDoneEvent(self.track.getName())
        self.acceptOnce(self.track.getDoneEvent(), self.fsm.request, ['neutral'])
        self.track.start(ts)
    ...
    def enterThrowGear(self, point, ts = 0):
        ...
        if hasattr(self, 'uniqueName'):
            ...
            name = 'vpThrowGear'
        self.track = Sequence(NPCLookInterval(self.vp_torso_node, lookNode, blendType='easeInOut', isBackwards=False), Func(VicePresident.throwGear, self, point), Func(base.playSfx, throwSfx), ActorInterval(self, 'throw'), name=name)
        self.track.setDoneEvent(self.track.getName())
        self.acceptOnce(self.track.getDoneEvent(), self.fsm.request, ['neutral'])
        self.track.start(ts)
        lookNode.removeNode()
    ...
    def throwGear(self, point):
        ...
        if self.gearThrowIval:
            ...
            self.gearThrowIval = None
        self.gearThrowIval = NPCWalkInterval(self.gearModel, point, durationFactor=0.01, fluid=1, lookAtTarget=False)
    ...
    def generateLegs(self):
        ...
        self.loadAnims({'stand-angry': 'phase_9/models/char/bossCog-legs-Fb_neutral.bam',
         ...
         'downhit': 'phase_9/models/char/bossCog-legs-Fb_firstHit.bam'}, 'legs')
    ...
    def generateBody(self):
        ...
        self.loadAnims({'stand-angry': 'phase_9/models/char/bossCog-torso-Fb_neutral.bam',
         ...
         'downhit': 'phase_9/models/char/bossCog-torso-Fb_firstHit.bam'}, 'body')

    def generateHead(self):
        self.loadModel('phase_9/models/char/sellbotBoss-head-zero.bam', 'head')
        self.loadAnims({'stand-angry': 'phase_9/models/char/bossCog-head-Fb_neutral.bam',
         ...
         'downhit': 'phase_9/models/char/bossCog-head-Fb_firstHit.bam'}, 'head')