# Embedded file name: lib.coginvasion.suit.DistributedSuit
...
class DistributedSuit(Suit, DistributedAvatar, DistributedSmoothNode, DelayDeletable):
    ...
    def __init__(self, cr):
        ...
        self.suitFSM = ClassicFSM('DistributedSuit', [State('off', self.enterSuitOff, self.exitSuitOff),
         ...
         State('bossFlying', self.enterBossFlying, self.exitBossFlying)], 'off', 'off')
    ...
    def enterBossFlying(self, startIndex, endIndex, ts = 0.0):
        ...
        if self.moveIval:
            self.moveIval.finish()
            self.moveIval = None
        self.animIval = Sequence(Func(self.animFSM.request, 'flyaway', [ts]), Wait(1.0), Func(self.animFSM.request, 'flydown', [ts]))
        self.moveIval = Sequence(Wait(0.5), Func(self.headsUp, endPos), ProjectileInterval(self, startPos=startPos, endPos=endPos, gravityMult=0.25, duration=duration))
        self.moveIval.start(ts)
