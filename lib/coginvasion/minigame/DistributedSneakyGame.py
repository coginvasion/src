# Embedded file name: lib.coginvasion.minigame.DistributedSneakyGame
"""
...
class DistributedSneakyGame(DistributedMinigame.DistributedMinigame):
    ...
    AreaModelParents = [render,
     ...
     'ENTRANCE',
     'EXIT']
    AreaModelPos = [Point3(0.0, 0.0, 0.0),
    ...
    def load(self):
        ...
        self.setDescription('Battle and defeat the other Toons with your water gun to gain points. ' + "Remember to reload your gun when you're out of ammo! " + 'The toon with the most points when the timer runs out gets a nice prize!')
    ...
    def enterCountdown(self):
        ...
        self.track = Sequence(Func(sec5.play), Func(text.setText, '5'), Wait(1.0), Func(sec4.play), Func(text.setText, '4'), Wait(1.0), Func(sec3.play), Func(text.setText, '3'), Wait(1.0), Func(sec2.play), Func(text.setText, '2'), Wait(1.0), Func(sec1.play), Func(text.setText, '1'), Wait(1.0), Func(text.destroy), Func(self.fsm.request, 'play'))
        self.track.start()
        del sec5
        del sec4
        del sec3
    ...
    def announceGenerate(self):
        ...
        self.load()
        base.camLens.setMinFov(CIGlobals.SneakyGameFOV / (4.0 / 3.0))
