# Embedded file name: lib.coginvasion.minigame.DistributedUnoGame
"""
...
class DistributedUnoGame(DistributedMinigame.DistributedMinigame):
    ...
    def deleteGui(self):
        if self.drawBtn:
            self.drawBtn.destroy()
            self.drawBtn = None
        if self.callBtn:
            self.callBtn.destroy()
            self.callBtn = None
        if self.playByPlay:
            self.playByPlay.destroy()
            self.playByPlay = None
    ...
    def createNewColorGui(self):
        ...
        self.newColorBlue = DirectButton(image=gui.find('**/mg_uno_numcards_blue_blank'), relief=None, scale=(0.2, 0.35, 0.35), pos=(-0.325, 0, -0.075), command=self.d_takeNewCardColor, extraArgs=[str(UGG.CARD_BLUE)])
        self.newColorRed = DirectButton(image=gui.find('**/mg_uno_numcards_red_blank'), relief=None, scale=(0.2, 0.35, 0.35), pos=(-0.125, 0, -0.075), command=self.d_takeNewCardColor, extraArgs=[str(UGG.CARD_RED)])
        self.newColorGreen = DirectButton(image=gui.find('**/mg_uno_numcards_green_blank'), relief=None, scale=(0.2, 0.35, 0.35), pos=(0.075, 0, -0.075), command=self.d_takeNewCardColor, extraArgs=[str(UGG.CARD_GREEN)])
        self.newColorYellow = DirectButton(image=gui.find('**/mg_uno_numcards_yellow_blank'), relief=None, scale=(0.2, 0.35, 0.35), pos=(0.275, 0, -0.075), command=self.d_takeNewCardColor, extraArgs=[str(UGG.CARD_YELLOW)])
        return

    def deleteNewColorGui(self):
        if self.newColorDialog:
            self.newColorDialog.destroy()
            self.newColorDialog = None
        if self.newColorTitle:
            self.newColorTitle.destroy()
            self.newColorTitle = None
        if self.newColorBlue:
            self.newColorBlue.destroy()
            self.newColorBlue = None
        if self.newColorRed:
            self.newColorRed.destroy()
            self.newColorRed = None
        if self.newColorGreen:
            self.newColorGreen.destroy()
            self.newColorGreen = None
        if self.newColorYellow:
            self.newColorYellow.destroy()
            self.newColorYellow = None
        return
    ...
    def placeCard(self, doId, cardId):
        ...
        if self.localAvId != doId:
            LerpPosInterval(card, duration=1.0, pos=(0, 0, 0.1), startPos=(-1.5, 0, 1.5), blendType='easeOut').start()
        else:
            self.numCards -= 1
            LerpPosInterval(card, duration=1.0, pos=(0, 0, 0.1), startPos=(0, 0, -1.5), blendType='easeOut').start()
            LerpScaleInterval(card, duration=1.0, scale=(0.4, 0.49, 0.49), startScale=(0.3, 0.3, 0.23), blendType='easeOut').start()
