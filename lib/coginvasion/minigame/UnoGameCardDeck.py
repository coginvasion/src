# Embedded file name: lib.coginvasion.minigame.UnoGameCardDeck
"""
...
class UnoGameCardDeck(DirectFrame):
    ...
    def updateCardToFollowGui(self):
        ...
        if self.unoGame.cardToFollow[-2:] == str(UGG.CARD_BLUE):
            self.cardToFollowGui = OnscreenImage(image=gui.find('**/mg_uno_numcards_blue_blank'), parent=base.a2dRightCenter)
        elif self.unoGame.cardToFollow[-2:] == str(UGG.CARD_RED):
            self.cardToFollowGui = OnscreenImage(image=gui.find('**/mg_uno_numcards_red_blank'), parent=base.a2dRightCenter)
        elif self.unoGame.cardToFollow[-2:] == str(UGG.CARD_GREEN):
            self.cardToFollowGui = OnscreenImage(image=gui.find('**/mg_uno_numcards_green_blank'), parent=base.a2dRightCenter)
        elif self.unoGame.cardToFollow[-2:] == str(UGG.CARD_YELLOW):
            self.cardToFollowGui = OnscreenImage(image=gui.find('**/mg_uno_numcards_yellow_blank'), parent=base.a2dRightCenter)
    ...
    def generate(self):
        ...
        numItemsVisible = 5
        itemHeight = -0.25
        self.deck = DirectScrolledList(itemFrame_relief=DGG.SUNKEN, decButton_pos=(0.35, 0, -0.02), decButton_geom=(gui.find('**/Horiz_Arrow_UP'),
         gui.find('**/Horiz_Arrow_DN'),
         gui.find('**/Horiz_Arrow_Rllvr'),
         gui.find('**/Horiz_Arrow_UP')), decButton_relief=None, decButton_hpr=(0, 0, 90), incButton_pos=(0.35, 0, 1.95), incButton_geom=(gui.find('**/Horiz_Arrow_UP'),
         gui.find('**/Horiz_Arrow_DN'),
         gui.find('**/Horiz_Arrow_Rllvr'),
         gui.find('**/Horiz_Arrow_UP')), incButton_hpr=(0, 0, -90), incButton_relief=None, pos=(-0.936, 0, -0.41), hpr=(0, 0, 90), scale=0.97, numItemsVisible=numItemsVisible, forceHeight=itemHeight, itemFrame_frameSize=(-0.2, 0.2, -0.37, 1.5), itemFrame_pos=(0.35, 0, 0.4), itemFrame_borderWidth=(0.02, 0.02))
        return
    ...
    def drawCard(self, id):
        ...
        card = cards.find('**/' + UGG.cardId2cardTex[id])
    ...
    def enableAll(self, cardToFollow = None):
        for btn in self.cardBtns:
            if cardToFollow != None:
                if cardToFollow == btn.getPythonTag('id') or cardToFollow[:2] == btn.getPythonTag('id')[:2] or cardToFollow[-2:] == btn.getPythonTag('id')[-2:] or btn.getPythonTag('id') == str(UGG.CARD_WILD) or btn.getPythonTag('id') == str(UGG.CARD_WILD_DRAW_FOUR):
