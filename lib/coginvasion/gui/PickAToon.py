# Embedded file name: lib.coginvasion.gui.PickAToon
"""
...
class PickAToon:
    ...
    def createGui(self):
        ...
        self.quit_btn = DirectButton(geom=(self.qtbtn.find('**/QuitBtn_RLVR'), self.qtbtn.find('**/QuitBtn_RLVR'), self.qtbtn.find('**/QuitBtn_RLVR')), relief=None, text='Quit', text_font=self.m, text_scale=0.105, text_pos=(0, -0.035), pos=(1.05, 0, -0.9), text_fg=(1, 0.9, 0.1, 1), geom1_scale=(1.02, 1, 1), geom2_scale=(1.02, 1, 1), command=self.quitGame)
        self.btnList = []
        ...
        for slot in range(6):
            if self.avChooser.hasToonInSlot(slot):
                ...
                delBtn = DirectButton(text=('', 'Delete', 'Delete', ''), text_scale=0.15, scale=0.5, text_pos=(0, -0.1), text_fg=(1, 1, 1, 1), relief=None, geom=(self.trash_gui.find('**/TrashCan_CLSD'), self.trash_gui.find('**/TrashCan_OPEN'), self.trash_gui.find('**/TrashCan_RLVR')), pos=self.trash_pos[slot], command=self.deleteToon, extraArgs=[self.avChooser.getAvChoiceBySlot(slot).getAvId()], text_shadow=(0, 0, 0, 1))
