...
class ChatInput(DirectObject, StateData.StateData):
    ...
    def __init__(self):
        ...
        self.fsm = ClassicFSM.ClassicFSM('chatInput', [State.State('off', self.enterOff, self.exitOff), State.State('idle', self.enterIdle, self.exitIdle), State.State('input', self.enterInput, self.exitInput)], 'off', 'off')
    ...
    def enter(self):
        ...
        self.chat_btn = DirectButton(text=('', 'Chat', 'Chat', ''), text_shadow=(0, 0, 0, 1), geom=(self.chat_btn_model.find('**/ChtBx_ChtBtn_UP'), self.chat_btn_model.find('**/ChtBx_ChtBtn_DN'), self.chat_btn_model.find('**/ChtBx_ChtBtn_RLVR')), relief=None, text_scale=0.0525, text_pos=(0, -0.08), text_fg=(1, 1, 1, 1), command=self.openChatInput, pos=(0.085, 0, -0.075), scale=1.25, parent=base.a2dTopLeft, extraArgs=[''])
